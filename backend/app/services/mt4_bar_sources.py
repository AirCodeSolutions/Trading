...
from app.services.mt4_csv import read_mt4_csv, read_recent_mt4_csv
...
_recent_csv_cache: dict[
    tuple[Path, Timeframe, int], tuple[int, list[MarketBar]]
] = {}
...
def load_recent_freshest_closed_bars(
    files_dir: Path,
    symbol: str,
    timeframe: Timeframe,
    evaluated_at: datetime,
    *,
    ...
    if snapshot.is_file():
        ...
        filtered = _closed_at(bars, evaluated_at, duration)
        if filtered:
            candidates.append((2, filtered[-limit:]))

    legacy = files_dir / f"{normalized}-{timeframe.value}.csv"
    if legacy.is_file():
        filtered = _closed_at(
            _cached_recent_csv_bars(
                legacy,
                normalized,
                timeframe,
                limit=limit,
        ...
        if filtered:
            candidates.append((1, filtered[-limit:]))
    ...
    if research.is_file():
        ...
        if filtered:
            candidates.append((0, filtered[-limit:]))
...
def _cached_recent_csv_bars(
    ...
    *,
    ...
    key = (path, timeframe, limit)
    cached = _recent_csv_cache.get(key)
    if cached is not None and cached[0] == mtime_ns:
        return cached[1]

    try:
        bars = read_recent_mt4_csv(
            path,
            symbol,
            timeframe,
            limit=limit,
    ...
    _recent_csv_cache[key] = (mtime_ns, bars)
    return bars
