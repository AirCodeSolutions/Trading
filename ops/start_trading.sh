...
if ! port_listening "$BACKEND_PORT"; then
  (
    cd "$BASE/backend"
    nohup .venv/bin/uvicorn app.main:app       --host 0.0.0.0       --port "$BACKEND_PORT"       >>"$RUNTIME/logs/backend.log" 2>&1 &
...
if ! pid_alive "$RUNTIME/pids/shadow-worker.pid"; then
  (
    cd "$BASE/backend"
    nohup .venv/bin/python -m app.shadow_worker       >>"$RUNTIME/logs/shadow-worker.log" 2>&1 &
...
if ! port_listening "$FRONTEND_PORT"; then
  (
    cd "$BASE/frontend"
    nohup node ./node_modules/vite/bin/vite.js       --host 0.0.0.0       --port "$FRONTEND_PORT"       >>"$RUNTIME/logs/frontend.log" 2>&1 &
