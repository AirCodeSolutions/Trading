...
def test_shadow_starts_fresh_without_backfill(
    ...
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow.load_causal_precursors",
        lambda *args, **kwargs: [_precursor(_bars())],
    )

    summary = advance_xau_compression_precursor_shadow_once(
        tmp_path,
        tmp_path,
        NOW,
    )
...
def test_future_precursor_is_resolved_once(
    ...
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow.load_recent_closed_market_bars",
        lambda *args, **kwargs: bars,
    )
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow.resolve_demo_sizing_capital",
    ...
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow.get_mt4_symbol_spec",
        lambda *args, **kwargs: _spec(),
    )
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow.load_research_execution_model",
        lambda *args, **kwargs: object(),
    )
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow.apply_research_execution_model",
        lambda spec, model: spec,
    )
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow.load_macro_events",
        lambda *args, **kwargs: [],
    )
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow.active_macro_blackouts",
        lambda *args, **kwargs: [],
    )
    ...
    monkeypatch.setattr(
        "app.services.xau_compression_precursor_shadow._simulate_candidate",
        lambda *args, **kwargs: (outcome, 20, None),
    )

    first = advance_xau_compression_precursor_shadow_once(
        tmp_path,
        tmp_path,
        NOW + timedelta(hours=3),
    )
    second = advance_xau_compression_precursor_shadow_once(
        tmp_path,
        tmp_path,
    ...
    assert len(load_xau_compression_precursor_shadow_summary(tmp_path).recent) == 1
